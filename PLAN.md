Got you. Just a **plan**, in English, no extra fluff, no agents.md (yet).
Also keeping in mind you’ll drive this mostly with an AI agent.
...
## 10. How to Use AI Agent Effectively for This

Since you’ll drive this via an agent, structure the work in **small, safe steps**:

1. **Step 1 – Project scaffold**

   * Ask agent to generate:

     * `scrapy startproject` command
     * Basic `settings.py` and empty files.

2. **Step 2 – Define schema (`items.py`)**

   * Give the agent the JSON spec (what you pasted) and ask:

     > “Create a Scrapy Item class that exactly matches this schema, including nested dicts/lists as Fields.”

3. **Step 3 – Implement spider skeleton**

   * Generate `AlkotekaSpider` with:

     * `name`
     * `allowed_domains`
     * `start_urls` or `start_requests()`
     * empty `parse` and `parse_product` with `pass`/TODO.

4. **Step 4 – Implement `parse`**

   * Tell agent:

     > “Using the markup of a sample category page, implement `parse` to extract product links and pagination.”

5. **Step 5 – Implement `parse_product` helpers**

   * Incrementally:

     * First fill simple fields (`timestamp`, `url`, `title`, `RPC`).
     * Then `price_data`.
     * Then `stock`.
     * Then `assets`.
     * Then `metadata`.
...
6. **Step 6 – Region and proxies**

   * Ask agent to:

     * Add cookie/header for Краснодар.
     * Implement a simple proxy middleware.

7. **Step 7 – Final pass & cleanup**

   * Ask agent to:

     * Check for PEP8.
     * Add minimal docstrings/comments.
